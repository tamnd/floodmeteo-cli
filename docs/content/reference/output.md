...
describes what users get. Pick a format with `-o`, or let floodmeteo choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
floodmeteo <command> -o table   # aligned columns for reading
floodmeteo <command> -o jsonl   # one JSON object per line, for piping
floodmeteo <command> -o json    # a single JSON array
floodmeteo <command> -o csv     # spreadsheet friendly
floodmeteo <command> -o tsv     # tab-separated
floodmeteo <command> -o url     # just the URL column
floodmeteo <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
floodmeteo <command> --fields id,title,url
```
...
```bash
floodmeteo <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
floodmeteo <command>            # a table, because this is a terminal
floodmeteo <command> | wc -l    # JSONL, because this is a pipe
```
